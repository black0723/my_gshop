  ...
  <div class="ratings" ref="ratings">
    <div class="ratings-content">
      ...
      <div class="ratingselect">
        <div class="rating-type border-1px">
          ...
          <span class="block positive" @click="setSelectType(0)" :class="{active:selectType===0}">
              满意<span class="count">{{satisfiedSize}}</span>
            </span>
          <span class="block negative" @click="setSelectType(1)" :class="{active:selectType===1}">
              不满意<span class="count">{{ratings.length - satisfiedSize}}</span>
            </span>
        </div>
      ...
      <div class="rating-wrapper">
        <ul>
          <li class="rating-item" v-for="(o,i) in filterRatings" :key="i">
            <div class="avatar">
              <img width="28" height="28" :src="o.avatar">
            </div>
            <div class="content">
              <h1 class="name">{{o.username}}</h1>
              <div class="star-wrapper">
                <Star :score="o.score" :size="24"/>
                <span class="delivery">{{o.deliveryTime}}</span>
              </div>
...
<script>
  ...
  export default {
    data() {
      return {
        onlyShowText: true,  //只看有内容的评论
        selectType: 2  //2代表全部，0满意，1不满意
      }
    },
    mounted() {
      this.$store.dispatch('getShopRatings', () => { //在异步数据加载完回调
        this.$nextTick(() => { //在界面显示完回调
          new BScroll(this.$refs.ratings, { //实例化滑动
            click: true
          })
        })
      })
    ...
    computed: {
      ...
      filterRatings() {
        ...
        return ratings.filter((o) => {
          const {rateType,text} = o
          /*
           条件1:
               selectType: 0/1/2
               rateType: 0/1
